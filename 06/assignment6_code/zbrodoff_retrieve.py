...
with model:
    ...
    actions = spa.Actions(
        ...
        # encode and retrieve
        # Calculation of utility of action on the left side of '-->':
        # Calculation of the dot product of goal and the ideal start vector (close to 1 when state of goal is start)
        # from the product, subtracting the declarative status which is zero at the beginning and .2 for small adjustment of the utility
        # Action taken is on the right side of '-->':
        # When selected, the action puts START*RETRIEVE into goal, slowly changing the 'state' to retrieve so that following actions are selected
        # Besides, the declarative state is filled with an addition of products between item and the argument which is given to the model.
        # This is used to retrieve information from memory
        a_retrieve='dot(goal,START) - declarative_status -.2 --> goal=START+RETRIEVE, declarative=ITEM1*arg1 + ITEM2*arg2',
        # Calculation of utility of action on the left side of '-->':
        # Once the state of goal is RETRIEVE, or a value close to it and the declarative value is increased, which it is after setting it in a_retrieve the utility of this action increases
        # Action taken is on the right side of '-->':
        # The goal buffer changes slowly into COMPARE and the answer state is set to the inverse of the ideal result vector multiplied by the value in the declarative module and four to adjust the value
        b_answer_retrieved='dot(goal,RETRIEVE) + declarative_status - .3 --> goal=(.8*RETRIEVE)+COMPARE, answer=4*~RESULT*declarative',

        # compare
        # Calculation of utility of action on the left side of '-->':
        # Once the goal buffer has shifted to compare, the dot product of goal and compare increases, so this action has higher utility.
        # Action taken is on the right side of '-->':
        # After executing, it uses the same function as in 'b_answer_retrieved' to slowly shift from COMPARE TO RESPOND, while setting up the values for comparison.
        # In comparison, target and answer are compared, both having different multiplication factors to adjust the value
        ...
        # Calculation of utility of action on the left side of '-->':
        # If RESPOND has a higher value in the goal state, the dot product increases and if then also the comparison has a positive value meaning it is correct, the utility increases
        # Use of the dot product of motor and yes is there to increase the signal since the comparison has fluctuations
        # Action taken is on the right side of '-->':
        # The motor state is set to yes since the comparison status was positive and the goal is shifted to DONE
        w_answer_yes='dot(motor, YES) + dot(goal, RESPOND) + comparison_status --> motor=YES, goal=DONE',
        # Calculation of utility of action on the left side of '-->':
        # If RESPOND has a higher value in the goal state, the dot product increases and if then the comparison has a negative value meaning it is false, the utility increases because of the '-' in the equation
        # Use of the dot product of motor and no is there to increase the signal since the comparison has fluctuations
        # Action taken is on the right side of '-->':
        # The motor state is set to no since the comparison status was negative and the goal is shifted to DONE
        x_answer_no=' dot(motor, NO) + dot(goal, RESPOND) - comparison_status --> motor=NO, goal=DONE',

       
        #finished
        # Calculation of utility of action on the left side of '-->':
        # This utility increases if DONE is in the goal state and the motor state has a value, either positive or negative
        # Action taken is on the right side of '-->':
        # As a result, the goal state increases the presence of the done vector
