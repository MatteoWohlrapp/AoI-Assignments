...
gain_D, bias_D = generate_gain_and_bias(N_D, -1, 1, max_rates[0], max_rates[1])
...
encoder_D = [np.random.choice([-1, 1]) for i in range(N_D)]
...
# calculate decoders for B with transformation of 0.5
decoder_BC = compute_decoder(encoder_B, gain_B, bias_B, function=function_BC)
# calculate decoders for D with no function for both dimensions
decoder_CD = compute_decoder(encoder_C, gain_C, bias_C)

# & combine with encoders for B
weightsAB = np.dot(decoder_AB, [encoder_B])
# & combine with encoders for C
weightsBC = np.dot(decoder_BC, [encoder_C])
# & combine with encoders for D
weightsCD = np.dot(decoder_CD, [encoder_D])
...
ideal_D = []
output_D = 0.0  # the decoded output value from population D
outputs_D = [[], []]  # -> 2 dimensional
...
while t < 10.0:
    ...
    # Calculate input for D for dimension 1 -> no need to calculate dimension zero, since it is only the stimulus value

    # decay all of the previous inputs (implementing the post-synaptic filter)
    for j in range(N_D):
        input_D[j] *= 1.0 - pstc_scale

    # for each neuron that spikes in C, increase the input current
    # of all the neurons it is connected to by the synaptic
    # connection weight
    for i, s in enumerate(spikes_C):
        if s:
            for j in range(N_D):
                input_D[j] += weightsCD[i][j] * pstc_scale

    # compute the total input into each neuron in population D
    # (taking into account gain and bias)
    total_D = [0] * N_D
    for j in range(N_D):
        total_D[j] = gain_D[j] * input_D[j] + bias_D[j]

    # run population D and determine which neurons spike
    spikes_D = run_neurons(total_D, v_D, ref_D)

    # for each neuron in D that spikes, update our decoded value
    # (also applying the same post-synaptic filter)
    output_D *= 1.0 - pstc_scale
    for j, s in enumerate(spikes_D):
        if s:
            output_D += decoder_D[j][0] * pstc_scale
    ...
    outputs_D[0].append(x)

    ideal_D.append(function_BC(function_AB(x)))
    outputs_D[1].append(output_D)
...
# plots the ensemble c
plt.figure()
plt.plot(times, ideal_C, label="ideal_C")
plt.plot(times, outputs_C, label="C")
plt.title("Simulation results for C")
plt.legend()

# plots the ensemble d
plt.figure()
plt.plot(times, outputs_D[0], label="ideal_D/stimulus, dimension 0")
plt.plot(times, ideal_D, label="ideal_D, dimension 1")
plt.plot(times, outputs_D[1], label="D, dimension 1")
plt.title("Simulation results for D")
plt.legend()
